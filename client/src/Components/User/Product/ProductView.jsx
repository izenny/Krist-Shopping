...
const ProductView = ({ product }) => {
  ...
  const handleColorSelect = (color) => {
    ...
    const isSizeAvailable = product?.stock.some(
      (item) =>
        item.color === color && item.size === selectedSize && item.quantity > 0
    );
    if (!isSizeAvailable) {
  ...
  const handleSizeSelect = (size) => {
    ...
    const isColorAvailable = product?.stock.some(
      (item) =>
        item.size === size && item.color === selectedColor && item.quantity > 0
    );
    if (!isColorAvailable) {
      ...
      <div className="md:w-1/2">
        <div className="md:w-3/4 flex flex-col gap-3">
          ...
          <p className="text-lg font-medium text-red-500">
            {product?.stock?.length === 0 ? "Out of Stock" : ""}
          </p>
          {/* Color Options */}
              ...
              {uniqueColors.map((color, index) => (
                ...
                ${
                 ...
                 ${
                  ...
                  onClick={() =>
                    availableColors.includes(color) && handleColorSelect(color)
                  }
                ></div>
              ))}
            </div>
          ...
          {product?.stock?.length !== 0 && (
            <div className="flex items-center gap-4 mt-4">
              ...
              {/* <button
                className="bg-black flex-1 text-white px-6 py-2 rounded-md hover:bg-gray-900"
